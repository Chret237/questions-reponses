...
public final class C$Gson$Types {
    ...
    private static Type resolve(Type type, Class<?> cls, Type type2, Collection<TypeVariable> collection) {
        ...
        if (!(type2 instanceof ParameterizedType)) {
            ...
            if (type2 instanceof WildcardType) {
                ...
                if (lowerBounds.length == 1) {
                    Type typeResolve3 = resolve(type, cls, lowerBounds[0], collection);
                    type3 = wildcardType;
                    if (typeResolve3 != lowerBounds[0]) {
                        return supertypeOf(typeResolve3);
                    }
                } else {
                    type3 = wildcardType;
                    if (upperBounds.length == 1) {
                        Type typeResolve4 = resolve(type, cls, upperBounds[0], collection);
                        type3 = wildcardType;
                        if (typeResolve4 != upperBounds[0]) {
                            return subtypeOf(typeResolve4);
                        }
                    }
                }
            }
        ...
        while (i < length) {
            Type typeResolve6 = resolve(type, cls, actualTypeArguments[i], collection);
            boolean z2 = z;
            Type[] typeArr = actualTypeArguments;
            if (typeResolve6 != actualTypeArguments[i]) {
                z2 = z;
        ...
        ParameterizedType parameterizedTypeNewParameterizedTypeWithOwner = parameterizedType;
        if (z) {
            parameterizedTypeNewParameterizedTypeWithOwner = newParameterizedTypeWithOwner(typeResolve5, parameterizedType.getRawType(), actualTypeArguments);
        }
