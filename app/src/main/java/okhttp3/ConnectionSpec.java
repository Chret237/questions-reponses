...
public final class ConnectionSpec {
    ...
    private ConnectionSpec supportedSpec(SSLSocket sSLSocket, boolean z) {
        ...
        String[] strArrConcat = strArrIntersect;
        if (z) {
            strArrConcat = strArrIntersect;
            if (iIndexOf != -1) {
                strArrConcat = Util.concat(strArrIntersect, supportedCipherSuites[iIndexOf]);
            }
        }
        return new Builder(this).cipherSuites(strArrConcat).tlsVersions(strArrIntersect2).build();
    }
