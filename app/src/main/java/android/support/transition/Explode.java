...
/* loaded from: classes-dex2jar.jar:android/support/transition/Explode.class */
public class Explode extends Visibility {
    ...
    private void calculateOut(View view, Rect rect, int[] iArr) {
        ...
        if (epicenter == null) {
            iCenterX = (view.getWidth() / 2) + i + Math.round(view.getTranslationX());
        ...
        float fRandom = fCenterX;
        float fRandom2 = fCenterY;
        if (fCenterX == 0.0f) {
            fRandom = fCenterX;
            fRandom2 = fCenterY;
            if (fCenterY == 0.0f) {
                fRandom = ((float) (Math.random() * 2.0d)) - 1.0f;
                fRandom2 = ((float) (Math.random() * 2.0d)) - 1.0f;
            }
        }
        float fCalculateDistance = calculateDistance(fRandom, fRandom2);
        float f = fRandom / fCalculateDistance;
        float f2 = fRandom2 / fCalculateDistance;
        float fCalculateMaxDistance = calculateMaxDistance(view, iCenterX - i, iCenterY - i2);
        iArr[0] = Math.round(f * fCalculateMaxDistance);
        iArr[1] = Math.round(fCalculateMaxDistance * f2);
    }
    ...
    public Animator onDisappear(ViewGroup viewGroup, View view, TransitionValues transitionValues, TransitionValues transitionValues2) {
        ...
        float translationX = view.getTranslationX();
        float translationY = view.getTranslationY();
        int[] iArr = (int[]) transitionValues.view.getTag(C0149R.id.transition_position);
        if (iArr != null) {
