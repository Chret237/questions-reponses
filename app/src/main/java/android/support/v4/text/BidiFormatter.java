...
public final class BidiFormatter {
    ...
    private static class DirectionalityEstimator {
        ...
        byte dirTypeBackward() {
            ...
            byte bSkipEntityBackward = cachedDirectionality;
            if (this.isHtml) {
                char c = this.lastChar;
                if (c == '>') {
                    bSkipEntityBackward = skipTagBackward();
                } else {
                    bSkipEntityBackward = cachedDirectionality;
                    if (c == ';') {
                        bSkipEntityBackward = skipEntityBackward();
                    }
                }
            }
            return bSkipEntityBackward;
        }
        ...
        byte dirTypeForward() {
            ...
            this.charIndex++;
            byte cachedDirectionality = getCachedDirectionality(this.lastChar);
            byte bSkipEntityForward = cachedDirectionality;
            if (this.isHtml) {
                char c = this.lastChar;
                if (c == '<') {
                    bSkipEntityForward = skipTagForward();
                } else {
                    bSkipEntityForward = cachedDirectionality;
                    if (c == '&') {
                        bSkipEntityForward = skipEntityForward();
                    }
                }
            }
            return bSkipEntityForward;
        }
