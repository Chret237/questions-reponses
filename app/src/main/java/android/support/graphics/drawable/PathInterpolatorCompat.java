...
/* loaded from: classes-dex2jar.jar:android/support/graphics/drawable/PathInterpolatorCompat.class */
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f21mX = new float[iMin];
        this.f22mY = new float[iMin];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < iMin; i2++) {
            pathMeasure.getPosTan((i2 * length) / (iMin - 1), fArr, null);
            this.f21mX[i2] = fArr[0];
            this.f22mY[i2] = fArr[1];
        }
        if (Math.abs(this.f21mX[0]) <= 1.0E-5d && Math.abs(this.f22mY[0]) <= 1.0E-5d) {
            int i3 = iMin - 1;
            if (Math.abs(this.f21mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f22mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < iMin) {
                    float[] fArr2 = this.f21mX;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        sb.append(" end:");
        int i5 = iMin - 1;
        sb.append(this.f21mX[i5]);
        sb.append(",");
        sb.append(this.f22mY[i5]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f21mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f21mX[i2]) {
                length = i2;
