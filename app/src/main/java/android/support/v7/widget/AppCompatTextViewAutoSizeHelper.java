...
class AppCompatTextViewAutoSizeHelper {
    ...
    private Method getTextViewMethod(String str) throws NoSuchMethodException, SecurityException {
        try {
    ...
    private boolean suggestedSizeFitsInSpace(int i, RectF rectF) {
        ...
        StaticLayout staticLayoutCreateStaticLayoutForMeasuring = Build.VERSION.SDK_INT >= 23 ? createStaticLayoutForMeasuring(charSequence, alignment, Math.round(rectF.right), maxLines) : createStaticLayoutForMeasuringPre23(charSequence, alignment, Math.round(rectF.right));
        return (maxLines == -1 || (staticLayoutCreateStaticLayoutForMeasuring.getLineCount() <= maxLines && staticLayoutCreateStaticLayoutForMeasuring.getLineEnd(staticLayoutCreateStaticLayoutForMeasuring.getLineCount() - 1) == charSequence.length())) && ((float) staticLayoutCreateStaticLayoutForMeasuring.getHeight()) <= rectF.bottom;
    }
    ...
    void loadFromAttributes(AttributeSet attributeSet, int i) {
        int resourceId;
        TypedArray typedArrayObtainStyledAttributes = this.mContext.obtainStyledAttributes(attributeSet, C0287R.styleable.AppCompatTextView, i, 0);
        if (typedArrayObtainStyledAttributes.hasValue(C0287R.styleable.AppCompatTextView_autoSizeTextType)) {
            this.mAutoSizeTextType = typedArrayObtainStyledAttributes.getInt(C0287R.styleable.AppCompatTextView_autoSizeTextType, 0);
        }
        float dimension = typedArrayObtainStyledAttributes.hasValue(C0287R.styleable.AppCompatTextView_autoSizeStepGranularity) ? typedArrayObtainStyledAttributes.getDimension(C0287R.styleable.AppCompatTextView_autoSizeStepGranularity, UNSET_AUTO_SIZE_UNIFORM_CONFIGURATION_VALUE) : -1.0f;
        float dimension2 = typedArrayObtainStyledAttributes.hasValue(C0287R.styleable.AppCompatTextView_autoSizeMinTextSize) ? typedArrayObtainStyledAttributes.getDimension(C0287R.styleable.AppCompatTextView_autoSizeMinTextSize, UNSET_AUTO_SIZE_UNIFORM_CONFIGURATION_VALUE) : -1.0f;
        float dimension3 = typedArrayObtainStyledAttributes.hasValue(C0287R.styleable.AppCompatTextView_autoSizeMaxTextSize) ? typedArrayObtainStyledAttributes.getDimension(C0287R.styleable.AppCompatTextView_autoSizeMaxTextSize, UNSET_AUTO_SIZE_UNIFORM_CONFIGURATION_VALUE) : -1.0f;
        if (typedArrayObtainStyledAttributes.hasValue(C0287R.styleable.AppCompatTextView_autoSizePresetSizes) && (resourceId = typedArrayObtainStyledAttributes.getResourceId(C0287R.styleable.AppCompatTextView_autoSizePresetSizes, 0)) > 0) {
        ...
        if (this.mAutoSizeTextType == 1) {
            if (!this.mHasPresetAutoSizeValues) {
                ...
                float fApplyDimension = dimension2;
                ...
                float fApplyDimension2 = dimension3;
                if (dimension3 == UNSET_AUTO_SIZE_UNIFORM_CONFIGURATION_VALUE) {
                    fApplyDimension2 = TypedValue.applyDimension(2, 112.0f, displayMetrics);
                }
                float f = dimension;
                if (dimension == UNSET_AUTO_SIZE_UNIFORM_CONFIGURATION_VALUE) {
                    f = 1.0f;
                }
                validateAndSetAutoSizeTextTypeUniformConfiguration(fApplyDimension, fApplyDimension2, f);
            }
            setupAutoSizeText();
        }
    }
